...
var testLocation,
  testprefix = 'arm-cli-vm-disk-encryption-windows-cs-restart',
        ...
        testUtils.executeCommand(suite, retry, cmd, function(result) {
          ...
          testUtils.executeCommand(suite, retry, cmd, function(result) {
            ...
            testUtils.executeCommand(suite, retry, cmd, function(result) {
              ...
              // vm restart
              var cmd = util.format('vm restart --resource-group %s --name %s --subscription %s --json', groupName, vmName, subscriptionId).split(' ');
              testUtils.executeCommand(suite, retry, cmd, function(result) {
                result.exitStatus.should.equal(0);
