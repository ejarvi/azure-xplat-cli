...
var testLocation,
  testprefix = 'arm-cli-vm-disk-encryption-windows-cs-internalrestart',
...
describe('arm', function () {
  describe('compute', function () {
    ...
    describe('vm', function () {
      it('should VmQuickCreateWindows-EncryptUsingClientSecret-GetStatusEncrypted-VmRestart-GetStatusEncrypted', function (done) {
        ...
        testUtils.executeCommand(suite, retry, cmd, function(result) {
          ...
          testUtils.executeCommand(suite, retry, cmd, function(result) {
            ...
            testUtils.executeCommand(suite, retry, cmd, function(result) {
                ...
                // vm restart triggered from within the vm 
                var csePublicConfig = '{"fileUris":["https://adetestsa.blob.core.windows.net/scripts/RemoteRestart.ps1"],"commandToExecute":".\\RemoteRestart.ps1"}';
                var cmd = util.format('vm extension set --resource-group %s --vm-name %s --name CustomScriptExtension --publisher-name Microsoft.Compute --version 1.8 --auto-upgrade-minor-version --public-config %s', groupName, vmName, csePublicConfig).split(' ');
                testUtils.executeCommand(suite, retry, cmd, function(result) {
                    result.exitStatus.should.equal(0);
